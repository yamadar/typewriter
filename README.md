...
- **扇形タイプバスケット**：蓋の扇形の穴から型かごが見え、打鍵すると**そのキーに対応するアームが振り上がる**（キーの左右位置＝アームの左右位置に対応）。
- **行間スイッチ**（蓋の右端・3 ポジション）：上から **青 = 1.0 / 白 = 1.5 / 赤 = 2.0**。切り替えると以後の LF の行送り量が変わる（切替時に「パチッ」と鳴る）。
...
- **効果音は実音をスペクトル解析して合成**：Enter ＝レジの「チン」／用紙解放＝ページめくり／マージンベル＝自転車ベル。
...
| 行間切替（1.0 / 1.5 / 2.0） | 蓋右端のスイッチ（青／白／赤） |
| 後退（重ね打ち） | Backspace |
