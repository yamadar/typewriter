...
- **行間スイッチ**（蓋右端 `#lhSwitch`、3ポジション）：上から **青=1.0 / 白=1.5 / 赤=2.0**。クロムのノブが選択位置へスライド、切替時に**「パチッ」**（`sndSwitch`）。`lineHeight`→`lineAdvance()=round(FS*lineHeight)`。各行の縦位置は累積 `rowTop[]` に保持（行ごとの行間を記録）。
