...
- **キーの機構**：`#mech` はマシン全体に重ねる透明オーバーレイ canvas。各キーの**アーム／タイプバーが、キー中心から紙の固定打点（印字点）へ向かって上方へ収束**する（集束点＝印字点に一致）。アームは**キーより背面**に描画（`.key` の z-index > `.mech`）＝キーの上に被らず、隙間とキー上方にのみ見える。打鍵でそのキーのタイプバーが赤く点灯して印字点へ振り上がり、打点でインクが弾ける。
