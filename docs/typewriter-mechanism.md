...
- **用紙解放レバー**（右側 `#paperRelease`）：引くとローラーが解放され、**タイプした紙だけ**を提示する表示に切替（機械はフェード）。もう一度引くと復帰。
- **外観（アンティーク風）**：キーは**黒い円形・白字・金属リングの縁影**。配列は実機準拠＝Backspace(◂◂)はPの右、Shift LockはAの左、左ShiftはZの左、右Shiftは `?/` の右、Spaceは幅広の単独行。
- **キーの機構（扇形のタイプバスケット）**：`#mech` はマシン全体に重ねる透明オーバーレイ canvas。実機写真どおりの**扇形（folding fan）**で表現：
  - **扇形の穴**＝カバーに開いた開口（暗い機械内部）。その奥に機構が見える。
  - **銀色のスラグ（ハンマー頭）**＝扇の外周（広い下弧）に並ぶ。
  - **タイプバー（骨）**＝外周から中心へ放射状に伸びる細線。
  - **ハブ（扇子の持ち手）**＝印字点直下の**実体のある丸い支え部品**（線ではない）。骨を座らせる。
  - 各キーの**リンクは真上→スラグへ**（キー背面 `.key` z-index>`.mech`）。打鍵でそのキーの銀スラグが弧の外側から**印字点へ振り上がり**、打点でインク。集束点＝印字点。
- **Shift Lock の状態反映**：Shift Lock を掛けるとシフトが機械的に保持されるため、両 Shift キーも点灯（held）する（`physDown || latch || isShiftLocked()`）。
- **マージンベル**：右端の手前（`bellCol = cols-8`）に達すると1行に1度チリーンと鳴る（CRで再アーム）。音は倍音つきの共鳴ベル（`sndBell`）。
- **CRレバーの操作**：右へ引く＝CR、手前（下）へ引く＝LF、ただ引く（クリック）＝CR+LF。**CRで右へスイング／LFで手前に倒れるアニメーション**（短い機構フィードバックなので reduced-motion でも動かす）。プラテンノブ(`#knobLF`)は補助のLFで、LF時に回転。
