...
- **キー下の機構**：キーボード下の `#mech` canvasに、各キーの**アーム／タイプバーが共通の打点（type basket の集束点）へ収束**する様子を描画。打鍵でそのキーのハンマーが打点へ振り上がる。
- **CRレバーの操作**：右へ引く＝CR、手前（下）へ引く＝LF、ただ引く（クリック）＝CR+LF。プラテンノブ(`#knobLF`)は補助のLF。
